...
\noindent\textbf{Affected locations:} Sections 2.3--2.4, pp.~15--25; Chapter IV opening and plot captions, pp.~49 and 52--54.
...
\begin{itemize}
  ...
  \item Avoiding an FFT capture array does not mean that the implementation contains no memory or buffering. The streaming design uses state, delay elements, lookup tables, and pipeline registers. At one accepted sample per clock, a fully serial sweep over $N_v$ rate candidates and $N_k$ center-frequency candidates requires approximately $NN_vN_k$ sample clocks plus control and pipeline overhead.

  \item A $W$-bit numerically controlled oscillator represents a requested frequency with the nearest frequency-control word, giving quantum $F_s/2^W$. With $W=16$ and $F_s=1$ MHz, the quantum is $15.2588$ Hz. Therefore the requested 1-Hz grid in Experiment XI does not demonstrate 1-Hz hardware resolution: its 201 requests map to only 14 distinct settings. The two discrete-time endpoints $-F_s/2$ and $+F_s/2$ also alias to the same oscillator setting.

  \item Contemporaneous component documentation supports the thesis design intent of a fixed two-tap filter with coefficients $[0.5,0.5]$. A later recovered active source tree contains a 41-tap, 128-bank filter and cannot be used as the historical RTL implementation of that two-tap description.

  \item The recovered modules are fixed-latency pipelines, not a fully elastic ready/valid interface. Several paths and state machines do not stall when the downstream ready signal is low. The documented characterization therefore assumes continuous operation with one valid sample per clock and a ready downstream path.
\end{itemize}

\section{Recovered-RTL limitations}

\noindent\textbf{Affected locations:} Sections 3.3.2, 3.3.4, 3.3.6, 3.3.8, and 3.3.9, pp.~32, 36, 40, and 44--48.

The exact thesis-era RTL revision is unavailable. The following observations apply to a later, source-divergent recovered checkout and therefore do not prove that the thesis-era simulations contained the same defects. They do prevent that checkout from independently establishing bit-true agreement with the equations.

\begin{itemize}
  \item The instantaneous-power path retains unsigned bits 29 through 14 and then reinterprets the resulting 16-bit word as signed. For 16-bit inputs with 15 fractional bits, powers at or above approximately $0.5$ can therefore appear negative, and an ideal power of $1.0$ is carried by a discarded upper bit and maps to zero.

  \item In the recovered accumulator, the sample-count generic is unused and the internal width is fixed at 28 bits. The 16-bit output slice \texttt{Sum(27 downto 12)} applies a 12-bit right shift, not division by $N=2^{16}$. A worst-case overflow-free sum of $2^{16}$ signed 16-bit samples requires 32 bits.

  \item When the end-of-block marker is asserted on sample $N-1$, the emitted sum excludes that marked sample, which instead seeds the next block. Several status signals can also remain asserted across invalid cycles rather than forming one-cycle handshake events. Boundary, scaling, width, and interface control require correction and re-verification before this checkout is reused.
\end{itemize}
