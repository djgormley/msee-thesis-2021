...
\begin{itemize}
  ...
  \[
    \mu_X(t)=\Expect[X(t)]=\int_{-\infty}^{\infty}x\,p_{X(t)}(x)\,\mathrm{d}x
  \]
  ...
  \[
    R_X(\tau)=\Expect\left[X\!\left(t+\frac{\tau}{2}\right)
  ...
  \item Energy spectral density, power spectral density, and the periodogram are distinct. For an energy signal $x(t)$, the energy spectral density is $|\mathcal{F}\{x(t)\}|^2$. For a wide-sense stationary power process, the power spectral density is the Fourier transform of its autocorrelation. A periodogram is a finite-record estimate and is not a consistent power-spectral-density estimator without suitable averaging or smoothing. \mbox{Parseval's} relation equates total time-domain energy with integrated spectral energy; it does not identify a pointwise spectrum with power.

  \item The spectral correlation function (SCF), also called the cyclic spectrum or cyclic spectral density, describes second-order periodic correlation. It does not specify a process's complete probability law. Its $\alpha=0$ slice is the ordinary power spectral density under the same conventions.

  \item The linear-modulation expression is
  \[
    X(t)=\sum_{k=-\infty}^{\infty}a_k p(t-kT_0-t_0)
    \mathrm{e}^{\mathrm{j}(2\pi f_c t+\theta_0)},
  \]
...
\begin{itemize}
  ...
  \[
    z_k[n]=\sum_m h[m]y[n-m]
      \mathrm{e}^{-\mathrm{j}2\pi f_k(n-m)/F_s}.
  \]
  Its estimated zero-lag cyclic-autocorrelation coefficient and nonnegative test statistic are
  \[
    \widehat{C}_y(\alpha_v,f_k)=\frac{1}{N}\sum_{n=0}^{N-1}|z_k[n]|^2
      \mathrm{e}^{-\mathrm{j}2\pi\alpha_v n/F_s},
    \qquad
    T_y(v,k)=|\widehat{C}_y(\alpha_v,f_k)|^2.
  \]
  This is a channelized, filter-localized cyclic statistic, not literally a point value of the SCF.
  ...
  \[
    \gamma_{\mathrm{CFD}}=\beta\widehat P_y^{\,2},\qquad \beta=10^{-4},
  \]
  where $\widehat P_y^{\,2}$ is the normalized sample-power-squared quantity that was mislabeled $\sigma_\eta^4$ in Table 12. The center-frequency-estimator testbench writes the statistic; the threshold overlay and decision are performed in post-processing. The value of $\beta$ is empirical rather than a calibrated probability of detection or false-alarm rate.
...
\begin{itemize}
  ...
  \item The datapath uses an AXI4-Stream contract with \texttt{aclk}, active-low \texttt{aresetn}, and \texttt{s\_axis} and \texttt{m\_axis} channels. A transfer occurs only when the channel's \texttt{tvalid} and \texttt{tready} are both asserted. Upstream \texttt{s\_axis\_tready} reflects the stage's capacity to accept a sample, and the source holds \texttt{m\_axis\_tdata}, \texttt{m\_axis\_tlast}, and \texttt{m\_axis\_tvalid} stable during downstream backpressure. Sample-preserving stages advance data and tags together, while integrate-and-dump stages incorporate every accepted input transfer into record state and emit one aligned result per record.
\end{itemize}
...
\begin{itemize}
  \item For Q1.15 complex input components, the instantaneous-power sequence $p_x[n]=x_{\mathrm{I}}[n]^2+x_{\mathrm{Q}}[n]^2$ is nonnegative and lies in $[0,2]$, with a full-precision unsigned 32-bit representation and 30 fractional bits. The interface into the signed Fourier mixer preserves the magnitude bit by zero-extending into a wider signed word or by explicit rescaling and saturation. One conforming 16-bit mapping is a saturated Q1.15 representation of $p_x[n]/2$; this gives a one-half scale factor in the complex CAF coefficient and a one-quarter factor in its magnitude-squared statistic and matching threshold.

  \item The selected-frequency Fourier output is the normalized coefficient $\widehat{x}(f_0)=N^{-1}\sum_{n=0}^{N-1}x[n]\mathrm{e}^{-\mathrm{j}2\pi f_0n/F_s}$. Both component sums include sample $N-1$ before a single division by $N$, quantization, and emission.

  \item The two-tap FIR uses zero initial delay state for every candidate record. Within a record, the delay holds the preceding accepted mixed sample. Acceptance of the transfer carrying \texttt{s\_axis\_tlast} marks the record boundary and clears the delay before the next record's first transfer. No state from one candidate enters the next.
  ...
  \item On each accepted input transfer, the accumulator first forms $s_{\mathrm{next}}=s+x[n]$. An ordinary sample stores that value. A sample carrying \texttt{s\_axis\_tlast} at count $N-1$ emits the normalized $s_{\mathrm{next}}$, asserts \texttt{m\_axis\_tvalid} and \texttt{m\_axis\_tlast}, and clears the stored sum for the next record. Without an accepted input transfer, sample-indexed state is unchanged; a pending output remains stable until accepted on \texttt{m\_axis}. This convention includes every record sample exactly once.
\end{itemize}
...
\begin{itemize}
  ...
  \item It repairs the MATLAB plotting scripts, uses fixed random seeds where stochastic examples are shown, corrects formulas and search grids, and regenerates the figures with shared typography, line weights, non-color visual cues, and the color-blind-friendly Okabe--Ito palette. Redundant in-plot titles are removed in favor of thesis captions, the three-dimensional plots use light translucent fills, and the legends for Figures 15 and 16 are placed outside the plotting axes.
  \item It plots the 2-, 4-, 8-, and 16-QAM-SRRC SCF estimates under the minimum-distance-two convention on one vertical scale, making the $1{:}2{:}6{:}10$ symbol-variance scaling explicit, and aligns the full SCF estimate's threshold plot with the viewing axes of the other three-dimensional spectra.
  ...
  \item It applies conventional DSP and FPGA symbols to the functional diagrams, uses \texttt{tikz-timing} for the timing diagrams, and uses AXI4-Stream interface names consistently throughout the corrected thesis.
