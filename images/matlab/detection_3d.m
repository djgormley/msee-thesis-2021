...
detected_rows = find((alpha(:) > 0) & (slice_peaks > gamma_CFD));
...
threshold_handle = surf(ax, frequency/1e3, alpha(display_rows)/1e3, ...
    gamma_CFD*ones(nnz(display_rows), num_freqs), ...
...
ylim(ax, [Rs, Fs/2]/1e3);
set(ax, 'ZScale', 'log');
zlim(ax, [gamma_CFD/10, 1.2*max(T_y(display_rows, :), [], 'all')]);
legend(ax, [mesh_handle, threshold_handle, peak_handle], ...
    {'$T_y^{\alpha}(f)$', '$\gamma_{\mathrm{CFD}}=\beta\widehat{P}_y^2$', ...
...
for idx = 1:numel(detected_rows)
    fprintf('Detected alpha = %.1f kHz at f = %.1f kHz (peak/gamma_CFD = %.2f)\n', ...
        detected_alpha(idx)/1e3, detected_frequency(idx)/1e3, ...
        slice_peaks(detected_rows(idx))/gamma_CFD);
end
non_target_rows = (alpha > 0) & (alpha ~= Rs);
fprintf('Largest non-target peak/gamma_CFD = %.3f\n', ...
    max(slice_peaks(non_target_rows))/gamma_CFD);
